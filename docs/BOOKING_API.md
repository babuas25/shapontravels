...
Completed unresolved Book results remain `202` but include safe `reason`, `nextAction: "contact_support"`, `statusUrl` and `automaticRetryAllowed: false`. Supplier duplicate/business failures, timeouts and invalid responses are distinct diagnostics; raw messages remain private. Clients must stop automatic polling and request review when `nextAction` is `contact_support`. Active pending work uses `check_saved_status`; pending rows older than five minutes switch to support guidance. A supplier error alone is not proof that no airline reservation exists, so the original reservation remains blocked. Same-key replay only retrieves it. New attempts persist diagnostic codes for operators; historical records are not rewritten with inferred causes.

The reservation transaction rechecks current client active state, booking scope and managed API eligibility after acquiring its authority/client lock. A revoked grant committed while Book was waiting returns `403 CLIENT_BOOKING_DISABLED` before creating a new supplier intent. Direct intent additionally requires current ticketing scope. This does not cancel work whose durable reservation already committed.
...
A verified PNR read replaces stored `ticketing_time_limit` with `lastTicketTime`, which takes priority over the original Book deadline. Supported values include the legacy `MM/dd/yyyy HH:mm:ss`, explicit RFC3339 timestamps, and the operator-confirmed Bangladesh local format `19 Sep 2026, 12:44 PM`. The latter resolves to `2026-09-19T12:44:00+06:00` (`Asia/Dhaka`). The original string remains in `lastTicketTime`; additive `lastTicketTimeIso` and `lastTicketTimeZone` expose its interpretation. Explicit offsets are honored; ambiguous legacy numeric values remain raw with null normalization metadata. A missing, null, empty or unparseable latest deadline clears the old stored deadline instead of treating it as current. Failed/mismatched responses cannot replace the stored deadline. Overlapping observations are ordered by database dispatch timestamp; an older lookup cannot overwrite a newer persisted observation. Evidence/deadline updates and a redacted `booking.pnr` audit event commit atomically. This priority and timezone rule does not itself authorize Issue. The Admin workflow in migration 0013 additionally records whether the latest supplier evidence passed validation, so failed/mismatched responses are not displayed as verified status.

`X-Booking-State` remains the local booking outcome. `X-Manual-Resolution-Required` is true for unresolved bookings, a supplier status other than `Booked` or `Created`, or ticket-number evidence. The held-status interpretation agrees with ticket eligibility. PNR success does not promote an uncertain booking to held or clear a pricing discrepancy. The existing local status/idempotent Book response remains the original stored outcome; use `/api/pnr` for current supplier status/deadline. Manual resolution is provided by the Admin workflow below; current Cancel behavior is documented in [held cancellation](CANCELLATION_API.md).

Migration `0028_booking_pnr_observations.sql` adds immutable response/validation history for explicit PNR reads. Each response is recorded under the booking lock, even when it completes out of order; existing latest-attempt display fields still follow request-time ordering. Held-ticket issue reads the latest verified historical observation, so a later failed check cannot erase known cancelled/ticketed evidence. It makes no new PNR call. The migration backfills only the observation still present at upgrade time; older overwritten evidence is unavailable.
