...
Implemented locally on 2026-09-11. Real supplier cancellation is disabled: both cancellation and its reconciliation require application environment `test` and an offline-capable transport. Real adapters inherit unconditional denial, even with existing booking/ticketing flags enabled. No real booking was cancelled during development.
...
Migration 0020 is applied only to disposable test databases. Working/production migration, commit/push and deployment remain pending. No live Book, Issue or Cancel test was run.
