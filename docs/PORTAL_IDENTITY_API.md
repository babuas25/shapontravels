...
- Other active users may read their own profiles. B2B owners and sub users cannot edit personal/company/bank fields themselves: current `lib/company-profile-access.ts` makes their whole profile read-only. Older field-lock comments do not override this actual guard.
...
| `documents/remove` | `operation_id`, target/slot, expected slot/identity versions. Manager-only company correction; increments a null-reference tombstone. Replay returns the current slot and cannot erase a later replacement. Storage history is retained. |
