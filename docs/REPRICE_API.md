...
| 503 | `REPRICE_BUSY`; `Retry-After: 1`, use bounded backoff with jitter |
...
Admission is process-wide, before opening the pricing transaction: one active RePrice per client, at most `min(4, floor(DB_MAX_CONNECTIONS / 2))` in total. Configure at least two database connections (default ten). No admission queue consumes database connections. A full gate returns 503 `REPRICE_BUSY`. Another process holding the selected offer lock also returns promptly with the same error through `NOWAIT`. Permits are released on completion/error/cancellation.

An admitted RePrice still keeps the offer/version transaction during its bounded supplier read, preserving serialization with acceptance and Book. Admission limits this connection use; it does not eliminate the transaction or provide a distributed global limit. Other endpoints still share database capacity.
