...
- `X-Search-Partial`: true if an active connection failed/timed out, a supplier budget blocked dispatch, or offers were excluded because their markup scope could not be verified. Intentionally disabled connections are not counted. All active connection calls start concurrently from a database snapshot.
...
Markup scope uses the **first requested route** and the **first flight segment's `airlineCode` in that route**, for one-way, connecting, return and multicity fares. `platingCarrier`, later segment airlines and codeshare flags do not choose the markup airline. For DAC→SIN via KUL with MH first and SQ second, the scope is MH + DAC→SIN. Every route alternative must match requested endpoints and connections by exact airport or a directly shared backend-configured city/metro group. Missing optional direction labels may use complete segment endpoints. Actual segment airports are required. When alternatives in the first route have different or missing/invalid first airlines, airline-specific candidates remain guarded by `COMPLEX_SCOPE_MATCHING_UNRESOLVED`; airline-independent rules can still use the validated route.

Search excludes an entire offer whose scope is unresolved **before** supplier-price selection and persistence, while retaining other verified offers and setting `X-Search-Partial: true`. Airport alternatives and changes within any of the 133 configured groups are retained. Airports without a directly shared group and ambiguous airlines remain guarded. Overlapping memberships are preserved without transitive merging: JFK/TTN and TTN/PHL match their respective groups, but JFK/PHL do not. Country alone never establishes equivalence; the catalog does not establish an included/protected ground transfer or a fallback markup. No individual alternative inside a priced offer is removed or rewritten. Monetary, passenger, currency, branded-fare and reference guards still apply to every source offer. If every nonempty offer is excluded, Search retains `422 COMPLEX_SCOPE_MATCHING_UNRESOLVED` and persists no inventory; a genuinely empty supplier result remains a successful empty Search. Summary counts and prices describe only retained offers. RePrice uses the same configured geography resolver and still requires the exact selected flight airports, airlines, numbers and departure times; even a same-city airport substitution is rejected.

The embedded `src/airport_groups.json` is generated from all entries in backend-owned `data/city-airport-groups.json`, including single-airport groups. `data/airports.json` provides audit diagnostics; `data/airport-group-reviews.json` is historical metadata, not an activation list. Missing/conflicting raw records and stale reviews are reported without disabling configured groups. Invalid group structure and conflicting group country assignments are rejected. Catalog maintenance uses `python3 scripts/audit-airport-mapping.py` and requires no frontend checkout. Group membership is an application policy, not a claim of verified real-world geography. Original supplier airports/references are preserved. The portal displays actual segment endpoints and labels airport changes in the result card and itinerary details. See [mapping evidence and maintenance](evidence/SEARCH_SCOPE_ISOLATION_2026-09-21.md).
