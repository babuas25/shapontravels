...
- RFC3339 deadlines with an explicit offset are checked locally. Verified PNR values in the confirmed `19 Sep 2026, 12:44 PM` format are interpreted as Bangladesh local time (`Asia/Dhaka`, UTC+06:00) and take priority over Book's deadline. The dispatch timeout plus 30 seconds must fit before the resolved deadline. Missing, empty, unsupported and ambiguous numeric local formats are retained verbatim and left for supplier validation. Book's offset-free strings do not inherit the PNR-specific timezone rule. No PNR call is added to Issue; explicit `/api/pnr` refresh retrieves and saves the current deadline. Without a refresh, a supplier-side cancellation or changed deadline may first be discovered by NewTicket.
...
- **202:** durable `pending` or `outcome_unknown`, with `issueId`, `bookingId` and `requiresReconciliation=true`; or verified ticket evidence whose wallet settlement is unresolved. Treat supplier business failure as potentially unresolved too. Never send another supplier issue request to try to fix it.
...
Unresolved Issue replies include `reason`, `nextAction`, `statusUrl` and `automaticRetryAllowed:false`. Only `TICKETING_IN_PROGRESS` permits bounded saved-status polling. Pending records older than five minutes (`TICKETING_STATUS_STALE`), completed unknown outcomes, and `WALLET_SETTLEMENT_REQUIRED` use `nextAction:contact_support`; stop automatic polling and retain the existing operation. `SUPPLIER_RECORD_LOCATOR_NOT_FOUND` reports the exact observed Triplover failure; other supplier rejections use `SUPPLIER_REPORTED_FAILURE`. A received response that fails verification uses `TICKETING_RESPONSE_UNVERIFIED`. Missing retained responses use `TICKETING_OUTCOME_UNKNOWN`: the saved evidence does not distinguish a timeout from other transport failures. Raw supplier text is never returned. None of these codes proves non-issuance or permits release of a wallet hold.
