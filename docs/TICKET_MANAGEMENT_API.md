...
For refund or void, omit `reissuePreferences` (or send `[]`). The backend reads ticket numbers, identity and financial entitlement from saved booking/issuance records. Requests require verified issued tickets with a captured wallet charge and supported per-passenger entitlement. Native, direct, IMP/EXP, manual and supplier imports use the same actions. Imports require a confirmed saved ticket and matching captured import charge. Accepted Rust pricing supplies per-passenger entitlement; older imports allocate the captured total using saved passenger fare weights with deterministic cent rounding (a single passenger receives the captured total). Unpaid or ambiguous historical tickets cannot invent refundable credit. Another active request on the same passenger is rejected. VOID intake follows the existing issue-date cutoff at 23:30 Bangladesh time; staff must still verify actual airline eligibility.
