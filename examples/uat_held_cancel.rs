//! Explicit opt-in continuation of the retained BG multicity UAT Hold. Never Book or Issue.
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("idempotency-key", "authorized-held-cancel-20260911")
...
#[tokio::main]
async fn main() {
    let phase = std::env::args().nth(1).expect("check, cancel or verify");
    assert!(["check", "cancel", "verify"].contains(&phase.as_str()));
    assert_eq!(
        std::env::var("AUTHORIZED_UAT_CANCELLATION").as_deref(),
        Ok("yes")
    );
    let database = std::env::var("UAT_CANCEL_DATABASE_URL").unwrap();
    ...
    supplier.ticketing_enabled = false; // No ticket issue is permitted in this process.
    let currency = supplier.currency.clone();
    let mut adapter = SupplierAdapter::new(supplier, Duration::from_secs(60)).unwrap();
    ...
    let dir = std::path::PathBuf::from(format!(
        ".local/evidence/uat-held-cancel-{}-{}",
        phase,
    ...
    sqlx::query("UPDATE api_clients SET permissions=$2 WHERE id=$1")
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE supplier_connections SET ticketing_enabled=false,servicing_enabled=true WHERE id='triplover'").execute(&pool).await.unwrap();
    ...
    let (status, body) = if phase == "check" {
        ...
        call(&app, &token, "/api/Cancel", input).await
    };
    save(&dir, "public-response", &body);
    let (original,preflight):(Option<Value>,Option<Value>)=sqlx::query_as("SELECT t.original_response,b.last_reconciliation FROM flight_bookings b LEFT JOIN flight_cancellations t ON t.booking_id=b.id WHERE b.id=$1").bind(id).fetch_one(&pool).await.unwrap();
    ...
    let summary = json!({"phase":phase,"httpStatus":status,"error":body["error"],"state":body["state"],"pnrStatus":preflight["item1"]["status"],"deadline":preflight["item1"]["lastTicketTime"],"cancelled":body["item1"]["isCancel"]});
    save(&dir, "summary", &summary);
    if ["cancel", "verify"].contains(&phase.as_str()) && [200, 202].contains(&status) {
        let retrieved = call(
            &app,
            &token,
        ...
        .await;
        save(&dir, "cancellation-retrieval", &retrieved.1);
        if phase == "cancel" {
        ...
        .await;
        save(&dir, "after-cancel-pnr", &pnr);
        println!(
            "After-cancel PNR HTTP {pnr_code}; status={}",
            pnr["item1"]["status"]
        );
    }
