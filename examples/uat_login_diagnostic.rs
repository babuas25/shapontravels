...
async fn main() {
 ...
 assert_eq!(base.trim_end_matches('/'),"https://userapi-uat.triplover.com");
 ...
 let client=reqwest::Client::builder().redirect(reqwest::redirect::Policy::none()).timeout(std::time::Duration::from_secs(30)).build().unwrap();
 let response=client.post(format!("{}/api/user/apiLogIn",base.trim_end_matches('/'))).json(&json!({"email":email,"password":password})).send().await.unwrap();
 let status=response.status().as_u16();
 let v:serde_json::Value=response.json().await.unwrap_or(serde_json::Value::Null);
 let msg=v["message"].as_str().unwrap_or("").replace(&password,"[REDACTED]").replace(&email,"[ACCOUNT]");
