//! One explicitly authorized UAT-only hold, using private passenger input.
...
const DIR: &str = ".local/evidence/uat-normal-hold-20260908";
...
async fn main() {
    ...
    if phase == "prepare" {
        let request = json!({"routes":[{"origin":"DAC","destination":"SIN","departureDate":"2026-11-01"}],"adults":1,"childs":1,"infants":0,"childrenAges":[3],"cabinClass":1,"preferredCarriers":[],"prohibitedCarriers":[]});
        ...
        println!(
            "Search offers={} eligible_consistent_hold={}",
            offers.len(),
            candidates.len()
        );
        for (i, offer) in candidates.into_iter().take(3).enumerate() {
            ...
            if body.pointer("/item2/isSuccess") != Some(&json!(true))
                ...
                || number(&fare["totalPrice"]) != number(&offer["totalPrice"])
            {
                continue;
            }
