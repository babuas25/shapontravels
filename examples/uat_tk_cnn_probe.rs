...
const DIR: &str = ".local/evidence/uat-tk-cnn-20260908-retry";
...
async fn investigate(config: shapontravels_api::config::SupplierConfig) {
    ...
    let cases = vec![("onewaycnn", true)];
    for (trip, cnn) in cases {
        let mut routes =
            vec![json!({"origin":"DAC","destination":"SIN","departureDate":"2026-11-01"})];
        if trip.starts_with("roundtrip") {
            routes.push(json!({"origin":"SIN","destination":"DAC","departureDate":"2026-11-15"}));
        }
        if trip == "multicity" {
            routes.extend([
                json!({"origin":"KUL","destination":"DAC","departureDate":"2026-11-15"}),
...
#[tokio::main]
async fn main() {
    assert!(std::env::args().any(|x| x == "--uat-search-reprice-only"));
    dotenvy::dotenv().ok();
