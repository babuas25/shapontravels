...
pub(super) async fn dispatch(
    ...
    tokio::spawn(async move {
        ...
        let saved = sqlx::query_as::<_,Booking>("SELECT id,public_ref,request_hash,state,public_response,supplier_id,original_response->'item2' AS supplier_result,error_code,(created_at<clock_timestamp()-INTERVAL '5 minutes') AS pending_stale,(SELECT state FROM flight_ticket_issues WHERE booking_id=$1) AS ticket_state,(SELECT state FROM flight_cancellations c WHERE c.booking_id=flight_bookings.id) AS cancellation_state FROM flight_bookings WHERE id=$1").bind(id).fetch_one(&mut *tx).await?;
        tx.commit().await?;
