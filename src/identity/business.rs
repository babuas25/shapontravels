...
async fn principal(
    ...
    let row: Principal = sqlx::query_as("SELECT u.id,u.clerk_user_id subject,u.role,u.authorization_version,m.agency_id,a.agency_code,o.clerk_user_id owner_subject,coalesce(nullif(trim(concat_ws(' ',u.first_name,u.last_name)),''),u.clerk_user_id) name,coalesce(u.email,'') email,coalesce(p.fields->>'agencyName','') agency_name FROM portal_users u LEFT JOIN portal_agency_memberships m ON m.user_id=u.id LEFT JOIN portal_agencies a ON a.id=m.agency_id LEFT JOIN portal_users o ON o.id=a.owner_user_id LEFT JOIN portal_identity_profiles p ON p.user_id=o.id AND p.kind='profile' WHERE u.id=$1")
