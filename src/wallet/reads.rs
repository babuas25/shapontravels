...
pub async fn statement(pool: &PgPool, account: Uuid, query: &ReadQuery) -> Result<Value> {
    ...
    let rows:Vec<(Value,i64)>=sqlx::query_as("SELECT jsonb_build_object('id',id,'sequence',sequence::text,'type',transaction_type,'amountMinor',amount::text,'currency',currency,'scale',2,'availableBeforeMinor',available_before::text,'availableAfterMinor',available_after::text,'holdBeforeMinor',hold_before::text,'holdAfterMinor',hold_after::text,'availableDeltaMinor',(available_after-available_before)::text,'holdDeltaMinor',(hold_after-hold_before)::text,'bookingId',booking_id,'bookingReference',coalesce(booking_reference,(SELECT coalesce(b.booking_reference,b.public_ref) FROM portal_import_bookings b JOIN wallet_operations w ON w.subject_kind='manual_issue' AND w.subject_id=b.id WHERE w.id=wallet_ledger_entries.operation_id)),'createdAt',created_at),sequence FROM wallet_ledger_entries WHERE wallet_account_id=$1 AND sequence<=$2 AND ($3::bigint IS NULL OR sequence<$3) AND ($4::timestamptz IS NULL OR created_at>=$4) AND ($5::timestamptz IS NULL OR created_at<$5) AND ($6::text IS NULL OR transaction_type=$6) ORDER BY sequence DESC LIMIT $7")
