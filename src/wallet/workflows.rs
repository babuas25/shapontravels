...
// The approved deposit entry is a one-to-one foreign-key relation. The staff
// directory fields are joined from the same database as the request rows. This
// lets the ledger page avoid a separate identity-directory round trip.
pub(super) const REQUEST_ROW: &str = "SELECT (to_jsonb(r)-'request_hash') || jsonb_build_object('owner_type',o.owner_type,'owner_key',o.owner_key,'owner_display',o.display,'current_balance',l.available_after::text,'requester_name',coalesce(nullif(trim(concat_ws(' ',requester.first_name,requester.last_name)),''),requester.email,r.requested_by_user_id),'reviewer_name',CASE WHEN r.reviewed_by_user_id IS NULL THEN NULL ELSE coalesce(nullif(trim(concat_ws(' ',reviewer.first_name,reviewer.last_name)),''),reviewer.email,r.reviewed_by_user_id) END,'reviewer_role',reviewer.role,'agency_name',CASE WHEN o.owner_type='agency' THEN coalesce(nullif(trim(agency_profile.fields->>'agencyName'),''),nullif(trim(concat_ws(' ',agency_owner.first_name,agency_owner.last_name)),''),agency_owner.email,o.display->>'name',o.owner_key) ELSE NULL END,'reversible_amount',CASE WHEN r.kind='deposit' AND r.status='approved' THEN (r.amount::numeric-(SELECT COALESCE(sum(x.amount),0) FROM wallet_requests x WHERE x.reversal_of=r.id AND x.status='approved'))::text ELSE NULL END,'reversal_deposit_ref',(SELECT d.public_ref FROM wallet_requests d WHERE d.id=r.reversal_of)) AS value FROM wallet_requests r JOIN wallet_accounts a ON a.id=r.wallet_account_id JOIN wallet_owners o ON o.id=a.owner_id LEFT JOIN wallet_ledger_entries l ON l.id=r.ledger_entry_id LEFT JOIN portal_users requester ON requester.clerk_user_id=r.requested_by_user_id LEFT JOIN portal_users reviewer ON reviewer.clerk_user_id=r.reviewed_by_user_id LEFT JOIN portal_agencies agency ON o.owner_type='agency' AND agency.agency_code=o.owner_key LEFT JOIN portal_users agency_owner ON agency_owner.id=agency.owner_user_id LEFT JOIN portal_identity_profiles agency_profile ON agency_profile.user_id=agency_owner.id AND agency_profile.kind='profile'";
fn request_wire(v: Value) -> Value {
    integer_strings(v, &["amount", "current_balance"])
}
